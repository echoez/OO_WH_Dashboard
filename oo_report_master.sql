      ...
      WHERE 1 = 1
        ...
        AND thing.relevent to cs = true
        AND s.is_over_sold IS NOT NULL
        AND s.is_hold = 1 then 'Hold'
        */
       
        /*SECURITY
        SELECT o.number, s.number, status, exported, f.name, l.name
        AND UPPER(adr.postal_code) in ('%EMPL', 'EMPL%')
        */
